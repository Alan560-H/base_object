...
class InviteUserController extends GetxController {
  ...
  saveImage() async {
    try {
      ...
      Uint8List? imageData = await _captureWidget(
        bannerKeys[currentSwiperIndex.value],
      );
      if (imageData != null) {
        final result = await ImageGallerySaverPlus.saveImage(
          imageData,
          quality: 100,
          name: "邀请好友${DateTime.now().millisecondsSinceEpoch}",
        );
  ...
  String get inviteUrl {
    String res =
        "${AppConfig.instance.registerUrl}?inviteCode=${userInfo.userModel.inviteCode}&downloadUrl=${Store.instance.getAppUpLoadModel.downUrl}?invite=${userInfo.userModel.inviteCode}&channelPackage=${Store.instance.getAppUpLoadModel.channelPackage}&logoUrl=${ImageConfig.logokh}";
